...
export function Terminal({ onPathChange, onRunProject, onLog }: TerminalProps) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter") {
      ...
      // Add input to lines
      const newLines: TerminalLine[] = [...lines, { type: "input", content: input, path: currentPath }]
      ...
      if (result.output.includes("__RUN__")) {
        ...
        setLines([
          ...
          { type: "output", content: 'Type your messages to interact with the AI. Type "exit" to return.' },
        ])
        setInput("")
        setHistory([...history, input])
        setHistoryIndex(-1)
  ...
  return (
    <div className="flex h-full flex-col bg-terminal-bg relative">
          ...
          <div key={idx} className="mb-1">
            {line.type === "input" ? (
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-sm text-terminal-fg">{line.content}</span>
              </div>
            ) : line.type === "error" ? (
              <div className="font-mono text-sm text-terminal-error">{line.content}</div>
            ) : (
              <div className="font-mono text-sm text-terminal-fg">{line.content}</div>
            )}
          </div>
        ))}
      ...
      {suggestions.length > 0 && (
        <div className="absolute bottom-2 left-4 right-4 rounded border border-terminal-border bg-terminal-bg/95 p-2 backdrop-blur shadow-lg">
          <div className="text-xs text-terminal-muted mb-1">Suggestions (press Tab):</div>
          {suggestions.slice(0, 5).map((sugg, idx) => (
            <div
              key={idx}
              className={`font-mono text-xs ${
                idx === suggestionIndex % suggestions.length ? "text-terminal-accent font-semibold" : "text-terminal-fg"
              }`}
            >
              {sugg}
            </div>
