...
export function Terminal({ onPathChange, onRunProject, onLog }: TerminalProps) {
  ...
  useEffect(() => {
    ...
    const offset = suggestions.length > 0 ? 5 * 20 : 0;
  ...
  return (
    <div className="flex h-full flex-col bg-terminal-bg relative">
      ...
      <div
        ...
        style={{
          // Reserve space for suggestions when they are visible
          paddingBottom: suggestions.length > 0 ? 6 * 20 : 16, // 5 suggestions * ~20px each
        }}
      >
        {lines.map((line, idx) => (
      ...
      {suggestions.length > 0 && (
        <div
          className="absolute left-4 right-4 rounded border border-terminal-border bg-terminal-bg/95 p-2 backdrop-blur shadow-lg"
          style={{
            // float above backend logs (assuming logs are h-64)
            bottom: 0,
          }}
        >
