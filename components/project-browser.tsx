...
export function ProjectBrowser({ currentPath, onPathChange, onOpenFile }: ProjectBrowserProps) {
  ...
  const renderNode = (node: FileNode, path: string, depth = 0) => {
    ...
    const isCurrent = currentPath === fullPath || (currentPath === "~" && fullPath === "~")
      ...
      <div key={fullPath}>
        <button
          ...
          className={`flex w-full items-center gap-2 rounded px-2 py-1.5 font-mono text-sm transition-colors ${
            isCurrent ? "bg-terminal-accent/20 text-terminal-accent" : "text-terminal-fg hover:bg-terminal-border/30"
          }`}
  ...
  return (
    <div className="flex w-80 flex-col bg-terminal-bg">
      <div className="border-b border-terminal-border px-4 py-2">
        <h3 className="font-mono text-xs font-semibold uppercase text-terminal-fg">File Browser</h3>
        <p className="mt-1 font-mono text-xs text-terminal-muted">{currentPath}</p>
      </div>
      <div className="terminal-scrollbar flex-1 overflow-y-auto p-2">
        {rootNode && rootNode.children && <>{rootNode.children.map((child) => renderNode(child, "~", 0))}</>}
      </div>
        ...
        if (currentNode?.metadata) {
          return (
            <div className="border-t border-terminal-border bg-terminal-border/20 p-4">
              <h4 className="mb-2 font-mono text-xs font-semibold uppercase text-terminal-fg">Project Info</h4>
              <p className="mb-2 font-mono text-xs text-terminal-fg">{currentNode.metadata.description}</p>
              ...
              {currentNode.metadata.runnable && (
                <div className="mt-3 font-mono text-xs text-terminal-success">✓ Runnable - type "run" in terminal</div>
              )}
            </div>
          )
        }
